...
expressions, etc.+ Non-expansive expressions are misleadingly referred
to as "syntactic values" (because they can be determined /by the parser/
[hence "syntactic"] as respecting the value-restriction side condition
for ~let~-expressions). Values are specified by Figure 13 (see Chapter 6) in
the 1997 Definition, but they loosely correspond to literal values
("special constants"), function closures, records whose entries are
values, some complicated rule for mutable references, and data
constructors whose payload is a value.
#+end_remark
